...
public class UserActionLogServiceImpl implements UserActionLogService {
    ...
    public void addUserAction(String username, String type, String action,
        ...
        try {
            ...
            log.warn("[UserActionLogServiceImpl] Failed to publish audit event action={}: {}", action, e.getMessage());
        }
    }
    ...
    private String resolveVdmsId() {
        if (cachedVdmsId == null) {
            try {
                ...
                log.warn("[UserActionLogServiceImpl] Could not fetch vdmsId from vdms-service: {}", e.getMessage());
            }
        }
