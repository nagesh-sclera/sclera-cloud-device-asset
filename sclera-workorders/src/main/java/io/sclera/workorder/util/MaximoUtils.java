...
/**
 * Maximo helper bean — per-VDMS token/URL cache + OSLC query builder + site catalog.
 *
 * Cache fields are keyed by vdmsId so concurrent requests for different VDMSes
 * do not overwrite each other's tokens or server URLs.
 */
@Component
public class MaximoUtils {
    ...
    private final ConcurrentHashMap<String, Long> tokenCreatedAtCache = new ConcurrentHashMap<>();
    private final ConcurrentHashMap<String, String> serverUrlCache = new ConcurrentHashMap<>();
    private final ConcurrentHashMap<String, String> siteIdCache = new ConcurrentHashMap<>();

    public String getToken(String vdmsId) {
        return tokenCache.get(vdmsId);
    }

    public void setToken(String vdmsId, String token) {
        tokenCache.put(vdmsId, token);
    }

    public Long getTokenCreatedAt(String vdmsId) {
        return tokenCreatedAtCache.get(vdmsId);
    }

    public void setTokenCreatedAt(String vdmsId, Long createdAt) {
        tokenCreatedAtCache.put(vdmsId, createdAt);
    }

    public String getServerUrl(String vdmsId) {
        return serverUrlCache.get(vdmsId);
    }

    public void setServerUrl(String vdmsId, String serverUrl) {
        serverUrlCache.put(vdmsId, serverUrl);
    }

    public String getSiteId(String vdmsId) {
        return siteIdCache.get(vdmsId);
    }

    public void setSiteId(String vdmsId, String siteId) {
        siteIdCache.put(vdmsId, siteId);
    }
    ...
    public Map<String, String> buildParams(String workOrderId, MaximoDTO maximoDTO, Integer pageno, Integer pagesize) {
        ...
        // Filter by siteid — array of objects e.g. [{"siteid":"IR"},{"siteid":"IR-F10"}]
        if (maximoDTO != null && maximoDTO.getSiteid() != null && !maximoDTO.getSiteid().isEmpty()) {
            String siteIdJson = maximoDTO.getSiteid().trim();
            try {
                ...
                log.error("Error parsing siteId JSON: {}", e.getMessage(), e);
    ...
    public JSONArray getSites() {
        ...
        return convertListOfMapToJsonArray(siteMap);
    }
