...
@NamedNativeQuery(
        name = "AssetSubCategory.getSubCategoryByCategoryId",
        query = "SELECT sc.id, sc.name, sc.icon_url AS iconUrl,sc.display_name AS displayName, sc.creation_timestamp AS creationTimestamp " +
                "FROM asset_sub_category sc " +
                "LEFT JOIN asset_category c ON c.id = sc.asset_category_id " +
                "WHERE ((sc.asset_category_id = ?1) " +
                "AND (?2 = 'all' OR CONCAT_WS('',sc.name,sc.display_name) LIKE CONCAT('%',?2,'%'))) " +
                "ORDER BY " +
                "CASE " +
                ...
                "   ELSE NULL " +
                "END ASC",
        resultSetMapping = "subCategoryMapping"
)

@NamedNativeQuery(
        name = "AssetSubCategory.getAllSubCategories",
        query = "SELECT id,name,icon_url AS iconUrl, display_name AS displayName, creation_timestamp AS creationTimestamp FROM asset_sub_category ",
        resultSetMapping = "subCategoryMapping"
)
...
public class AssetSubCategory {
    ...
    @ManyToOne
    @JoinColumn(referencedColumnName = "id", name = "asset_category_id")
    private AssetCategory assetCategory;
    @OneToOne(cascade = CascadeType.ALL, mappedBy = "assetSubCategory")
